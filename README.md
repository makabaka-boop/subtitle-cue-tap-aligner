...
2. 每条计划与每次敲击至多使用一次；
3. 仅考虑绝对偏差不超过**配对容差**的计划（默认 **800 ms**，可按本场在
   100–2000 ms 间调整，见下节）；
...
联排场地噪声或操作节奏变化时，操作员可在提交前为本场选择 **100–2000 ms**
的整数配对容差（页面默认仍为 800 ms）：

1. `POST /api/match` 增加可选字段 `tolerance_ms`；服务端以它替代固定
   800 ms 的候选窗口——升序处理、单次使用、最小偏差、平局取较早计划
   的裁决顺序不变，原始配对与锚点重算使用同一窗口；
2. 响应回显实际采用的 `tolerance_ms`，页面在结果区展示本次数值；
3. 容差不是整数或超出范围时，服务端返回 400 与明确中文原因
   （`TOLERANCE_NOT_INTEGER` / `TOLERANCE_OUT_OF_RANGE`），不产生新
   结果；页面保留输入值、忽略勾选与已有配对结果，修正后可再次提交；
4. 误触忽略仍按原始下标工作，不受窗口变化影响。

**不携带该字段（或 null）的旧请求仍按 800 ms 规则配对，响应与旧版本
逐字段完全一致（不含 `tolerance_ms` 键）。**
...
2. 服务端校验下标**唯一**且**位于本场敲击范围**后，从候选敲击中排除它们
   ——其余敲击仍按上述规则（含本场容差）配对，原始下标与序号**不重排**；
...
3. 对校准后的有效时间按上面的既有规则（升序、单次使用、本场容差范围、
